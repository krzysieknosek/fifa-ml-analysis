...
for model_name, model in models.items():
    ...
    print(f"Training MSE with best params: {train_mse:.2f}")

    val_pred = best_model.predict(X_val_prepared)
    val_mse = mean_squared_error(y_val, val_pred)
    print(f"Validation MSE with best params: {val_mse:.2f}")

    eval_pred = best_model.predict(X_eval_prepared)
    eval_mse = mean_squared_error(y_eval, eval_pred)
    print(f"Evaluation MSE with best params: {eval_mse:.2f}")
